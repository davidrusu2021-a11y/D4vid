...
# ??????????????????????????????????????????
BOT_TOKEN   = "MTUwNTU0NjI3MzkwOTExMjg3Mg.GtBHAp.ERnIO9oL6rsJSmtnVSD1PI_ZNuVdoxz7yx21jg"   # ?? Resetta il token sul Developer Portal prima!
...
async def redeem(interaction: discord.Interaction, chiave: str):
    ...
    if key_data["used"]:
        await interaction.response.send_message(embed=error_embed("Questa chiave ? gi? stata usata."), ephemeral=True)
        return

    user_id = str(interaction.user.id)

    # Controlla se gi? in whitelist
    if user_id in data["whitelist"]:
        await interaction.response.send_message(embed=error_embed("Sei gi? nella whitelist!"), ephemeral=True)
        return
...
async def getscript(interaction: discord.Interaction):
    ...
    e.add_field(name="?? Avviso", value="Non condividere questo messaggio con nessuno.", inline=False)
    await interaction.response.send_message(embed=e, ephemeral=True)
...
async def whitelist_add(interaction: discord.Interaction, utente: discord.User):
    ...
    if user_id in data["whitelist"]:
        await interaction.response.send_message(embed=error_embed(f"{utente.mention} ? gi? nella whitelist."), ephemeral=True)
        return
...
async def whitelist_remove(interaction: discord.Interaction, utente: discord.User):
    ...
    if user_id not in data["whitelist"]:
        await interaction.response.send_message(embed=error_embed(f"{utente.mention} non ? nella whitelist."), ephemeral=True)
        return
...
@tree.command(name="whitelist_check", description="Controlla se un utente ? nella whitelist")
...
class PanelView(discord.ui.View):
    ...
    async def gen_key_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != OWNER_ID:
            await interaction.response.send_message(embed=error_embed("Solo l'owner pu? farlo."), ephemeral=True)
            return
        data = load_data()
        key = gen_key()
